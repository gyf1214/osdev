...
void initFB() {
    ...
    device_t *device = (device_t *)kalloc(KmemDevice);
    device -> write = fbWrite;

    devfsMountDevice(devRoot, FBMountPoint, device);
}
...
ssize_t fbWrite(uint32_t info, const void *data, size_t n) {
    Unused(info);
